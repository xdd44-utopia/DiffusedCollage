...
def diffuse():
	...
	image = pipe(content['prompt'], negative_prompt=negative_prompt).images[0]
	image.save(f"../Assets/Images/{count}.png")
	image, _ = inference(f"../Assets/Images/{count}.png")
	pixels = image.load()
	emptyPixel = 0
	for i in range(image.size[0]):    # for every col:
		for j in range(image.size[1]):    # For every row
			if (pixels[i, j][3] < 10):
				emptyPixel += 1
	print(emptyPixel)
	if (emptyPixel > 250000):
		return jsonify({"fileName":f"{count}.png"})
	else:
		image.save(f"../Assets/Images/{count}_nobackground.png")
		return jsonify({"fileName":f"{count}_nobackground.png"})
